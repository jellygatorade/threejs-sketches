function RuleHead(string) {
  ...
  if (conditionIndex != -1) {
    if (predecessorIndex > conditionIndex) predecessorIndex = -1;

    if (successorIndex > conditionIndex) successorIndex = -1;
  }
  ...
  } else this.predecessor = null;

  this.symbol = new Symbol(string, index);

  if (successorIndex != -1)
    this.successor = new Symbol(string, successorIndex + 1);
  else this.successor = null;

  if (conditionIndex != -1) this.condition = string.substr(conditionIndex + 1);
...
function Rule(string) {
  ...
  this.head = new RuleHead(
    source.substring(0, source.lastIndexOf(this.SYMBOL_EQUALS))
  );
  this.body = new RuleBody(
    source.substring(source.lastIndexOf(this.SYMBOL_EQUALS) + 1)
  );
...
Rule.prototype = {
  ...
  isApplicable(symbol, predecessor, successor) {
    if (!symbol.matches(this.head.symbol)) return false;

    if (
      this.head.predecessor != null &&
      !this.head.predecessor.matches(predecessor)
    )
      return false;
...
function Symbol(string, index = null) {
  if (index != null) this.parse(string, index);
  else {
    this.symbol = string;
    this.parameters = [];
  }
...
Symbol.prototype = {
  parse(string, index) {
    ...
    if (index + 1 < string.length && string[index + 1] == "(") {
      ...
      while (string[++index])
        if (string[index] == "(") ++scope;
        else if (string[index] == ")" && --scope == 0) break;
  ...
  createFunctions(keys) {
    ...
    for (var parameter = 0; parameter < this.parameters.length; ++parameter)
      this.functions.push(
        new Function(keys, "return " + this.parameters[parameter])
      );
  },
  ...
  toString() {
    ...
    if (this.parameters.length != 0) {
      ...
      for (var parameter = 0; parameter < this.parameters.length; ++parameter)
        ...
        else str += this.parameters[parameter] + ",";
    }
...
Lindenmayer.prototype = {
  ...
  setRules(rules) {
    for (var rule = 0; rule < rules.length; ++rule) this.addRule(rules[rule]);
  },
      ...
      if (s.parameters.length > 0)
        for (var parameter = 0; parameter < s.parameters.length; ++parameter)
          result.parameters.push(
            s.functions[parameter].apply(this, Object.values(rule.key))
          );
  ...
  parseSymbol(predecessor, symbol, successor) {
    ...
    if (rules.length == 0) return [symbol];
  ...
  applyRules(sentence) {
    ...
    for (var symbol = 0; symbol < sentence.length; ++symbol) {
      ...
      if (symbol > 0) predecessor = sentence[symbol - 1];

      if (symbol + 1 < sentence.length) successor = sentence[symbol + 1];

      newSentence = newSentence.concat(
        this.parseSymbol(predecessor, sentence[symbol], successor)
      );
    }
